...
use Validator;
use DB;
class MerchantController extends JoshController
{
    ...
    public function data()
    {
        ...
        $users = User::select(['users.id', 'users.first_name', 'users.pic', 'users.last_name', 'oollah_user_details.verified_date', 'users.email', 'users.gender', 'users.postal','users.created_at'])
            ->join('role_users', 'role_users.user_id', '=', 'users.id')
            ->leftJoin('oollah_user_details', 'oollah_user_details.user_id', '=', 'users.id')
            ->where('role_users.role_id', 3)->orderby('id', 'desc');
        ...
        return Datatables::of($users)
            ...
            ->add_column('verified',function($user){
            ...
            ->add_column('actions',function($user) {
                ...
                if ((Sentinel::getUser()->id != $user->id) && ($user->id != 1)) {
                    $actions .= '<a href='. route('confirm-delete/merchant', $user->id) .' data-toggle="modal" data-target="#delete_confirm"><i class="livicon" data-name="user-remove" data-size="18" data-loop="true" data-c="#f56954" data-hc="#f56954" title="delete user"></i></a>';
                }
                return $actions;
            }
