...
class InviteHistoryController extends Controller
{
    ...
    {
        return View('admin/invitehistory/invitehistory');
    }
    ...
    {
        $tables = InviteHistory::orderby('id', 'desc');

        return Datatables::of($tables)
            ->edit_column('masterid', function ($data) {
                $masterUser = DB::table('users')->select(['users.username as username' ])->where('users.id', $data->masterid)->first();

                return $masterUser->username;
            })
            ->edit_column('receiverid', function ($data) {
                $receiverUser = DB::table('users')->select(['users.username as username' ])->where('users.id', $data->receiverid)->first();
    ...
    {
        DB::table('weebie_invite')->where('id', $id)->delete();
