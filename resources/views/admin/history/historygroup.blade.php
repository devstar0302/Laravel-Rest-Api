...
@section('content')
    <section class="content-header">
        <h1>{{ _t('Record History', [], $_SESSION['lang']) }}</h1>
        <ol class="breadcrumb">
            <li>
            ...
            <li><a href="#"> {{ _t('History', [], $_SESSION['lang']) }}</a></li>
            <li class="active">{{ _t('HistoryGroup', [], $_SESSION['lang']) }}</li>
        </ol>
    </section>
            ...
            <div class="panel panel-primary ">
                <div class="panel-heading">
                    <h4 class="panel-title"> <i class="livicon" data-name="spinner-six" data-size="16" data-loop="true" data-c="#fff" data-hc="white"></i>
                        {{ _t('Group Record History', [], $_SESSION['lang']) }}
                    </h4>
                </div>
                <br />
                    ...
                    <table class="table table-striped table-bordered table-hover dataTable no-footer sample_editable"
                        ...
                        <tr role="row">
                            ...
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   Group Name
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Group Name', [], $_SESSION['lang']) }}
                            </th>
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   Real Name
                                            : activate to sort column ascending" style="width: 100px;">{{ _t(config('Convert.realname')[0], [], $_SESSION['lang']) }}
                            </th>
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   File Name
                                            : activate to sort column ascending" style="width: 100px;">{{ _t(config('Convert.filename')[0], [], $_SESSION['lang']) }}
                            </th>
...
@section('footer_scripts')
    ...
    <script>
        ...
        $(function() {
            ...
            table = $('#notificationtable').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{!! route('admin.history.groupdata') !!}',
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'send_id', name: 'send_id' },
    ...
    <script>
        ...
        function deleteData(){
            ...
            $.ajax({
                type: "get",
                url: '/admin/history/' + id + '/groupdelete'
                success: function (result) {
                    alert(result);
                    if(result == ''){
