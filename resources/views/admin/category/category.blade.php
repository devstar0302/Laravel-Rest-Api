...
@section('content')
    ...
    <section class="content-header">
        <h1>{{ _t('Sections', [], $_SESSION['lang']) }}</h1>
        <ol class="breadcrumb">
            <li>
            ...
            <li class="active">{{ _t('Sections', [], $_SESSION['lang']) }}</li>
        </ol>
    </section>
    <!-- Main content -->
                        ...
                        <h3 class="panel-title">
                                    <span style="font-size: 110%">
                                     <i class="livicon" data-name="tasks" data-size="15" data-c="#fff" data-hc="#fff"
                                        data-loop="true"></i>
                                        {{ _t('Sections', [], $_SESSION['lang']) }}</span>
                        </h3>
                    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTable no-footer sample_editable"
                            ...
                            <tr role="row">
                                ...
                                <th class="sorting_asc" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                Name
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Section Name', [], $_SESSION['lang']) }}
                                </th>
                                ...
                                <th class="sorting" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                status
                                            : activate to sort column ascending" style="width: 50px;">{{ _t('Status', [], $_SESSION['lang']) }}
                                </th>
                                <th class="sorting" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                Edit
                                            : activate to sort column ascending" style="width: 50px;">{{ _t('Edit', [], $_SESSION['lang']) }}
                                </th>
                                <th class="sorting" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                Delete
                                            : activate to sort column ascending" style="width: 50px;">{{ _t('Delete', [], $_SESSION['lang']) }}
                                </th>
                            </tr>
                            </thead>
                            <tbody>
    ...
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <form role="form" action="{{ route('admin.category.add') }}" id="AddCategoryForm" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                <div class="modal-content" style="width: 80%">
                    <div class="modal-header" style="font-size: 150%">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="myModalLabel">{{ _t('Add Section', [], $_SESSION['lang']) }} </h4>
                    </div>
                    <div class="modal-body" style="font-size: 120%">
                        <table style="width:50%" class="table form-body" border="0">
                            <tr style="border:0">
                                <th style="border:0">{{ _t('Section Name', [], $_SESSION['lang']) }}:</th>
                                <td style="border:0">
                                    <input type="text" name="name" class="form-control" id="name"  value="" placeholder="{{ _t('Section name', [], $_SESSION['lang']) }}">
                                </td>
                            </tr>
                        </table>
                    </div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <form role="form" action="{{ route('admin.category.edit') }}" id="EditCategoryForm" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                    ...
                    <div class="modal-body" style="font-size: 120%">
                        <table style="width:50%" class="table form-body" border="0">
                            <tr style="border:0">
                                <th style="border:0">{{ _t('Section Name', [], $_SESSION['lang']) }}:</th>
                                <td style="border:0">
                                    <input type="text" name="name" class="form-control" id="edit_name"  value="" placeholder="{{ _t('Section name', [], $_SESSION['lang']) }}">
                                </td>
                            </tr>
                        </table>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-body">
                    {{ _t(config('Convert.category_delete_message')[0], [], $_SESSION['lang']) }}
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="deleteCategory()">{{ _t('Delete', [], $_SESSION['lang']) }}</button>
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal">{{ _t('Cancel', [], $_SESSION['lang']) }}</button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-body" id="statecontent">
                    {{ _t(config('Convert.category_active_message')[0], [], $_SESSION['lang']) }}
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="inactive_category()">{{ _t('OK', [], $_SESSION['lang']) }}</button>
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal">{{ _t('Cancel', [], $_SESSION['lang']) }}</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="inactiveModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-body" id="statecontent">
                    {{ _t(config('Convert.category_inactive_message')[0], [], $_SESSION['lang']) }}
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="active_category()">{{ _t('OK', [], $_SESSION['lang']) }}</button>
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal">{{ _t('Cancel', [], $_SESSION['lang']) }}</button>
                </div>
            </div>
        </div>
...
@section('footer_scripts')
    ...
    <script>
        ...
        $(function () {
            ...
            $('#citytable_length').append('<button type="button" style="margin-left: 20px; border:1px solid #ccc; background-color: #6c7def; color: white" class="form-control button-sm" data-toggle="modal" data-target="#addModal">{{ _t(config('Convert.add')[0], [], $_SESSION['lang']) }}</button >');
