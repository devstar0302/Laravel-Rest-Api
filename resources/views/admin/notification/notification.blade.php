...
{{-- Page title --}}
@section('title')
    {{ _t('Notifications', [], $_SESSION['lang']) }}
    @parent
@stop
...
@section('content')
    <section class="content-header">
        <h1>{{ _t('Notifications', [], $_SESSION['lang']) }}</h1>
        <ol class="breadcrumb">
            <li>
            ...
            <li><a href="#"> {{ _t('Notifications', [], $_SESSION['lang']) }}</a></li>
            <li class="active">{{ _t('Notifications', [], $_SESSION['lang']) }}</li>
        </ol>
    </section>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTable no-footer sample_editable"
                        ...
                        <tr role="row">
                            ...
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   Title
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Notification Title', [], $_SESSION['lang']) }}
                            </th>
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   Description
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Notification Description', [], $_SESSION['lang']) }}
                            </th>
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   expired
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Expired Date', [], $_SESSION['lang']) }}
                            </th>
                            <th class="sorting_asc" tabindex="0" aria-controls="paidtable" rowspan="1"
                                colspan="1"  aria-label="
                                                   Status
                                            : activate to sort column ascending" style="width: 100px;">{{ _t(config('Convert.status')[0], [], $_SESSION['lang']) }}
                            </th>
...
@section('footer_scripts')
    ...
    <script>
        ...
        function active_data(){
            ...
            $.ajax({
                type: "get",
                url: '/admin/notification/' + id + '/activedata',
        ...
        function inactive_data(){
            ...
            $.ajax({
                type: "get",
                url: '/admin/notification/' + id + '/inactivedata',
                success: function (result) {
        ...
        function addNotification(){
            location.href="{{ route('admin.notification.add') }}";
        }
        function deleteData(){
            var id = $('#deleteid').val();
            ...
            $.ajax({
                type: "get",
                url: '/admin/notification/' + id + '/delete',
                success: function (result) {
                    if(result == ''){
