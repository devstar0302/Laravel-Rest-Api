...
@section('content')
    ...
    <section class="content-header">
        <h1>{{ _t('Merchant Plan', [], $_SESSION['lang']) }}</h1>
        <ol class="breadcrumb">
            <li>
            ...
            <li class="active">{{ _t('Merchant Plan', [], $_SESSION['lang']) }}</li>
        </ol>
    </section>
    <!-- Main content -->
                        ...
                        <h3 class="panel-title">
                                    <span style="font-size: 110%">
                                     <i class="livicon" data-name="tags" data-size="15" data-c="#fff" data-hc="#fff"
                                        data-loop="true"></i>
                                        {{ _t('Merchant Plan', [], $_SESSION['lang']) }}</span>
                        </h3>
                    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTable no-footer sample_editable"
                            ...
                            <tr role="row">
                                ...
                                <th class="sorting_asc" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                Name
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Plan Name', [], $_SESSION['lang']) }}
                                </th>
                                <th class="sorting_asc" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                Plan unit
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Plan Unit', [], $_SESSION['lang']) }}
                                </th>
                                <th class="sorting_asc" tabindex="0" aria-controls="occasiontable" rowspan="1"
                                    colspan="1" aria-label="
                                                price
                                            : activate to sort column ascending" style="width: 100px;">{{ _t('Plan Price (USD)', [], $_SESSION['lang']) }}
                                </th>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-body" style="font-size: 120%">
                    ...
                    <table style="width:90%" class="table form-body" border="0">
                        <tr style="border:0">
                            <th style="border:0">{{ _t('Plan Name', [], $_SESSION['lang']) }}:</th>
                            <td style="border:0">
                                <input type="text" name="name" class="form-control" id="name"  value="" placeholder="{{ _t('plan name', [], $_SESSION['lang']) }}">
                            </td>
                        </tr>
                        <tr style="border:0">
                            <th style="border:0">{{ _t('Plan Unit', [], $_SESSION['lang']) }}:</th>
                            <td style="border:0">
                                <select name="expired" class="form-control" id="expired">
                                    <?php
                                    for($i = 0; $i < count($expireds); $i++){
                                        echo '<option value="'.$expireds[$i].'">'.$expireds[$i].'</option>';
                                    }
                                    ?>
                                </select>
                            </td>
                        ...
                        <tr style="border:0">
                            <th style="border:0">{{ _t('Plan Price (USD)', [], $_SESSION['lang']) }}:</th>
                            <td style="border:0">
                                <input type="number" name="price" step="0.01" class="form-control" id="price"  value="" placeholder="{{ _t('plan price', [], $_SESSION['lang']) }}">
                            </td>
                        </tr>
                    </table>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="addPlan()">{{ _t(config('Convert.add')[0], [], $_SESSION['lang']) }}</button>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="deletePlan()">{{ _t(config('Convert.delete')[0], [], $_SESSION['lang']) }}</button>
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal">{{ _t(config('Convert.cancel')[0], [], $_SESSION['lang']) }}</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="activeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="inactive_plan()">{{ _t(config('Convert.ok')[0], [], $_SESSION['lang']) }}</button>
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal">{{ _t(config('Convert.cancel')[0], [], $_SESSION['lang']) }}</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="inactiveModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content" style="width: 80%">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal" onclick="active_plan()">{{ _t(config('Convert.ok')[0], [], $_SESSION['lang']) }}</button>
                    <button type="button" class="btn btn-default" class="form-control" data-dismiss="modal">{{ _t(config('Convert.cancel')[0], [], $_SESSION['lang']) }}</button>
                </div>
            </div>
        </div>
...
@section('footer_scripts')
    ...
    <script>
        ...
        $(function () {
            ...
            $('#tagtable_length').append('<button type="button" style="margin-left: 20px; border:1px solid #ccc; background-color: #6c7def; color: white" class="form-control button-sm" data-toggle="modal" data-target="#addModal" onclick="addModal()">{{ _t(config('Convert.add')[0], [], $_SESSION['lang']) }}</button >');

            /*  Edit functionality  */
            var row_id, name, expired, created_date, price;
            ...
            function editRow(table, nRow) {
                ...
                expired = aData.expired ? aData.expired : '';
                price = aData.price ? aData.price : '';
                $('#planid').val(row_id);
                $('#name').val(name);
                $('#expired').val(expired);
                $('#price').val(price);
    ...
    <script>
        ...
        function addPlan () {
            ...
            if (name == "") {
                ...
                $('#messagecontent').html('{{ _t('Please input plan name.', [], $_SESSION['lang']) }}');
            ...
            if (price == "") {
                /*  make  modal- " Please input occasion name."  */
                $('#messagecontent').html('{{ _t('Please input plan price.', [], $_SESSION['lang']) }}');
                /*  display  modal- " Please input occasion name."  */
                $('#messageModal').modal('show');
                return;
            }
        ...
        function inactive_plan(){
            var id = $('#occ_id').val();
            console.log(id);
            // console.log(id);
            $.ajax({
                type: "get",
                url: '/admin/plans/' + id + '/inactive',
                success: function (result) {
